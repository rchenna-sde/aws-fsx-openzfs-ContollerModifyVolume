...
func (c *FakeCloudProvider) ModifyVolume(ctx context.Context, volumeId string, parameters map[string]string) (*Volume, error) {
	if c.volumes == nil {
...
func (c *FakeCloudProvider) TagResource(ctx context.Context, resourceId string, tags map[string]string) error {
	return nil
}

func (c *FakeCloudProvider) UntagResource(ctx context.Context, resourceId string, tagKeys []string) error {
	return nil
}

func (c *FakeCloudProvider) ModifyVolumeWithTags(ctx context.Context, volumeId string, parameters map[string]string, tags map[string]string, untagKeys []string) (*Volume, error) {
	// First modify the volume
	volume, err := c.ModifyVolume(ctx, volumeId, parameters)
	if err != nil {
		return nil, err
	}
	// Then apply tags (simulated)
	if len(tags) > 0 {
		err = c.TagResource(ctx, volumeId, tags)
		if err != nil {
			return nil, err
		}
	}
	// Finally remove tags (simulated)
	if len(untagKeys) > 0 {
		err = c.UntagResource(ctx, volumeId, untagKeys)
		if err != nil {
			return nil, err
		}
	}
...
func (c *FakeCloudProvider) WaitForVolumeModification(ctx context.Context, volumeId string) error {
	return nil
}

func (c *FakeCloudProvider) DetectExternalChanges(ctx context.Context, volumeId string, expectedParams map[string]string) (bool, error) {
	return false, nil
}

func (c *FakeCloudProvider) GetChangedParameters(ctx context.Context, volumeId string, requestedParams map[string]string) (map[string]string, error) {
